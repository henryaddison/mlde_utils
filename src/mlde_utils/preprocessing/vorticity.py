...
class Vorticity:
    ...
    def run(self, ds):
        ...
        if ds[f'xwind{self.theta}'].attrs['grid_mapping'] == "latitude_longitude":
            ...
            dx = dx[None, :]
            dy = dy[None, :]
            vort_da = mpcalc.vorticity(ds[f'xwind{self.theta}'], ds[f'ywind{self.theta}'], dx=dx, dy=dy)

        vort_da = vort_da.assign_attrs(grid_mapping=ds[f'xwind{self.theta}'].attrs['grid_mapping'], units="s-1", standard_name="atmosphere_relative_vorticity", long_name="relative_vorticity")
